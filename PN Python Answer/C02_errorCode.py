...
result.append({interface1_list[6]:interface1_list[-1]})

interface2_list = sys_log[1].split(" ")
result.append({interface2_list[6]:interface2_list[-1]})

login_list = sys_log[2].split(" ")
result.append({login_list[6]:login_list[-1]})

cmd_list = sys_log[3].split("=")
cmd_user = cmd_list[-1].split(";")[0]
cmd_ip = cmd_list[2][0:13]
